...
class editAddress extends Controller {
    ...
    public function modifyAddress() {
        if (isset($_POST['add_address_submit'])){
            ...
            $address->city = $_POST['city2'];
            $address->postal_code = $_POST['postal_code2'];
            $address->state = $_POST['state2'];
            $address->street = $_POST['street2'];
    ...
    public function loadAddresses() {
        ...
        for ($i=0; $i < count($this->address_list) ; $i++) { 
            $return_string .= '<div class="input-group input-group-lg">
            <input type="text" id="'.$this->address_list[$i]->id.'" readonly value="'.$this->address_list[$i]->street.', '.$this->address_list[$i]->city.', '.$this->address_list[$i]->state.'" class="form-control"><span class="input-group-btn"><button type="button" class="btn btn-default  secondary-color secondary-border-color secondary-hover"><span class="glyphicon glyphicon-pencil secondary-color" onCLick="editAddress('.$this->address_list[$i]->id.',\''.$this->address_list[$i]->street.'\',\''.$this->address_list[$i]->city.'\',\''.$this->address_list[$i]->state.'\',\''.$this->address_list[$i]->country.'\',\''.$this->address_list[$i]->postal_code.'\')" data-toggle="modal" data-target="#editModal"></span></button></span>
            '; 
            if (count($this->address_list) > 1) {
                $return_string .='<span class="input-group-btn"><button type="button" class="btn btn-default secondary-border-color secondary-hover"><span class="glyphicon glyphicon-remove" style="color:red;" onCLick="location.href=\'/editAddress/deleteAddress/'.$this->address_list[$i]->id.'\'"></span></button>';
            }
            $return_string .= '</div>';
        }
